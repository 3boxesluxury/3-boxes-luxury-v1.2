...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ─── v22: Fetch Instagram Business Account data via Facebook Pages ───
    // Instagram Graph API works through the Facebook Page → Instagram Business Account bridge
    // Flow: User's Facebook Pages → Page's Instagram Business Account → IG Profile + Media + Hashtags
    let instagramData: {
      ...
      recentMedia: Array<{ id: string; caption: string | null; mediaType: string; mediaUrl: string | null; permalink: string | null; timestamp: string | null }>;
    ...
    try {
      ...
      const pagesResponse = await fetch(
        `https://graph.facebook.com/${FB_GRAPH_VERSION}/me/accounts?fields=id,name,access_token,instagram_business_account&access_token=${accessToken}`,
        { method: 'GET' }
      );
      const pagesData = await pagesResponse.json();

      if (pagesData.data && pagesData.data.length > 0) {
        console.log('[Facebook OAuth] v22: Found', pagesData.data.length, 'Facebook Pages');

        // Step 2: Find a Page with an Instagram Business Account linked
        for (const page of pagesData.data) {
          if (page.instagram_business_account) {
            const igBusinessId = page.instagram_business_account.id;
            const pageAccessToken = page.access_token;
            console.log('[Facebook OAuth] v22: Found Instagram Business Account:', igBusinessId, 'on Page:', page.name);

            // Step 3: Fetch Instagram Business Account profile
            const igProfileResponse = await fetch(
              `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}?fields=id,username,biography,followers_count,follows_count,media_count,profile_picture_url&access_token=${pageAccessToken}`,
              { method: 'GET' }
            );
            const igProfileData = await igProfileResponse.json();

            if (igProfileData.id) {
              instagramData.igUserId = igProfileData.id;
              instagramData.username = igProfileData.username || null;
              instagramData.biography = igProfileData.biography || null;
              instagramData.followersCount = igProfileData.followers_count || null;
              instagramData.followsCount = igProfileData.follows_count || null;
              instagramData.mediaCount = igProfileData.media_count || null;
              instagramData.profilePictureUrl = igProfileData.profile_picture_url || null;

              // v22.8: If profile_picture_url not returned, try the /picture edge as fallback
              if (!instagramData.profilePictureUrl) {
                try {
                  const picResponse = await fetch(
                    `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}/picture?width=200&height=200&redirect=false&access_token=${pageAccessToken}`,
                    { method: 'GET' }
                  );
                  const picData = await picResponse.json();
                  if (picData.data && picData.data.url) {
                    instagramData.profilePictureUrl = picData.data.url;
                    console.log('[Facebook OAuth] v22.8: Got Instagram profile pic from /picture edge');
                  }
                } catch (picErr) {
                  console.warn('[Facebook OAuth] v22.8: /picture edge failed:', picErr);
                }
              }

              console.log('[Facebook OAuth] v22.8: Instagram profile:', instagramData.username, '| profile_picture_url:', instagramData.profilePictureUrl ? 'YES' : 'NONE');

              // Step 4: Fetch recent media (up to 25) for style analysis
              const igMediaResponse = await fetch(
                `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}/media?fields=id,caption,media_type,media_url,permalink,timestamp&limit=25&access_token=${pageAccessToken}`,
                { method: 'GET' }
              );
              const igMediaData = await igMediaResponse.json();

              if (igMediaData.data && igMediaData.data.length > 0) {
                instagramData.recentMedia = igMediaData.data.map((media: any) => ({
                  id: media.id,
                ...
                // Step 5: Extract hashtags from captions for style analysis
                const allHashtags: string[] = [];
                for (const media of instagramData.recentMedia) {
                  if (media.caption) {
                ...
                // Deduplicate and take top 30 most frequent hashtags
                const hashtagCounts: Record<string, number> = {};
                for (const tag of allHashtags) {
                  hashtagCounts[tag] = (hashtagCounts[tag] || 0) + 1;
                }
                instagramData.recentHashtags = Object.entries(hashtagCounts)
                  .sort(([, a], [, b]) => b - a)
                  .slice(0, 30)
                  .map(([tag]) => tag);

                console.log('[Facebook OAuth] v22: Fetched', instagramData.recentMedia.length, 'IG media,',
                  instagramData.recentHashtags.length, 'unique hashtags');
              }
            }

            // We found an Instagram account — no need to check more pages
            break;
          }
        }

        if (!instagramData.igUserId) {
          console.log('[Facebook OAuth] v22: No Instagram Business Account found on any Facebook Page');
        }
      } else {
        console.log('[Facebook OAuth] v22: No Facebook Pages found for this user');
      }

      // v22.9: FALLBACK — If no IG data found via /me/accounts, try using target_ids from token debug.
      // The token debug response contains granular_scopes with target_ids for Instagram scopes.
      // These target_ids ARE the Instagram Business Account IDs (IGIDs) we need.
      if (!instagramData.igUserId) {
        console.log('[Facebook OAuth] v22.9: No IG data via /me/accounts — trying token debug fallback...');
        try {
          const debugResponse = await fetch(
            `https://graph.facebook.com/debug_token?input_token=${accessToken}&access_token=${FACEBOOK_APP_ID}|${FACEBOOK_APP_SECRET}`,
            { method: 'GET' }
          ...
          if (debugData.data?.granular_scopes) {
            // Find the instagram_basic scope — its target_ids contain the IG Business Account IDs
            const igScope = debugData.data.granular_scopes.find(
              (scope: any) => scope.scope === 'instagram_basic' && scope.target_ids && scope.target_ids.length > 0
            );

            if (igScope && igScope.target_ids[0]) {
              const igBusinessId = igScope.target_ids[0];
              console.log('[Facebook OAuth] v22.9: Found IG Business Account ID from token debug:', igBusinessId);

              // Fetch Instagram profile directly using the IG Business Account ID
              // Note: We use the user access token here since we don't have a page token
              const igProfileResponse = await fetch(
                `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}?fields=id,username,biography,followers_count,follows_count,media_count,profile_picture_url&access_token=${accessToken}`,
                { method: 'GET' }
              );
              const igProfileData = await igProfileResponse.json();
              ...
              if (igProfileData.id) {
                ...
                // v22.9: /picture edge fallback for profile pic
                if (!instagramData.profilePictureUrl) {
                  try {
                    const picResponse = await fetch(
                      `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}/picture?width=200&height=200&redirect=false&access_token=${accessToken}`,
                      { method: 'GET' }
                    );
                    const picData = await picResponse.json();
                    if (picData.data && picData.data.url) {
                      instagramData.profilePictureUrl = picData.data.url;
                      console.log('[Facebook OAuth] v22.9: Got Instagram profile pic from /picture edge (token debug path)');
                    }
                  } catch (picErr) {
                    console.warn('[Facebook OAuth] v22.9: /picture edge failed (token debug path):', picErr);
                  }
                }

                console.log('[Facebook OAuth] v22.9: Instagram profile (token debug fallback):', instagramData.username, '| profile_picture_url:', instagramData.profilePictureUrl ? 'YES' : 'NONE');

                // Try to fetch recent media too
                try {
                  const igMediaResponse = await fetch(
                    `https://graph.facebook.com/${FB_GRAPH_VERSION}/${igBusinessId}/media?fields=id,caption,media_type,media_url,permalink,timestamp&limit=25&access_token=${accessToken}`,
                    { method: 'GET' }
                  );
                  const igMediaData = await igMediaResponse.json();
                  if (igMediaData.data && igMediaData.data.length > 0) {
                    ...
                    for (const media of instagramData.recentMedia) {
                      if (media.caption) {
                        const hashtags = media.caption.match(/#[\w]+/g);
                        if (hashtags) allHashtags.push(...hashtags.map((h: string) => h.toLowerCase()));
                      }
                    }
                    const hashtagCounts: Record<string, number> = {};
                    for (const tag of allHashtags) hashtagCounts[tag] = (hashtagCounts[tag] || 0) + 1;
                    instagramData.recentHashtags = Object.entries(hashtagCounts)
                      .sort(([, a], [, b]) => b - a)
                      .slice(0, 30)
                      .map(([tag]) => tag);
                    console.log('[Facebook OAuth] v22.9: Fetched', instagramData.recentMedia.length, 'IG media (token debug path)');
                  }
                } catch (mediaErr) {
                  console.warn('[Facebook OAuth] v22.9: Could not fetch IG media (token debug path):', mediaErr);
                }
              } else {
                console.log('[Facebook OAuth] v22.9: IG Business Account ID from token debug did not return profile data:', JSON.stringify(igProfileData));
              }
            } else {
              console.log('[Facebook OAuth] v22.9: No instagram_basic target_ids found in token debug');
            }
          } else {
            console.log('[Facebook OAuth] v22.9: No granular_scopes in token debug response');
          }
        } catch (debugErr) {
          console.warn('[Facebook OAuth] v22.9: Token debug fallback failed:', debugErr);
        }
      }
    } catch (igError) {
      console.warn('[Facebook OAuth] v22: Could not fetch Instagram data:', igError);
    }
    ...
    // v22: Added Instagram data (profile, media, hashtags) from Instagram Graph API
    const facebookExtendedData = JSON.stringify({
      ...
      instagram: instagramData.igUserId ? {
        ...
        recentHashtags: instagramData.recentHashtags,
      } : null,
    });

    // ─── CONNECT ACTION: Link provider to current user, do NOT change auth session ───
    if (isConnectAction) {
      // v22.6: Read oauth_instagram cookie to know which button the user clicked
      const isInstagramConnect = request.cookies.get('oauth_instagram')?.value === 'true';
      console.log('[Facebook OAuth] v22.6: Connect action — oauth_instagram cookie:', request.cookies.get('oauth_instagram')?.value, '→ isInstagramConnect:', isInstagramConnect);
      console.log('[Facebook OAuth] v22.6: Instagram data available:', !!instagramData.igUserId, instagramData.igUserId ? `username=${instagramData.username}` : 'none');

      const returnTo = request.cookies.get('oauth_return_to')?.value || '/';
      const redirectUrl = new URL(returnTo, BASE_URL);

      if (isInstagramConnect && instagramData.igUserId) {
        // User clicked "Connect Instagram" → set connect_provider=instagram
        redirectUrl.searchParams.set('connect_provider', 'instagram');
        redirectUrl.searchParams.set('connect_name', instagramData.username || 'Instagram');
        // Use Instagram profile picture if available, otherwise fall back to Facebook avatar
        const igAvatar = instagramData.profilePictureUrl || facebookAvatar || null;
        if (igAvatar) redirectUrl.searchParams.set('connect_avatar', igAvatar);
        redirectUrl.searchParams.set('connect_id', instagramData.igUserId);
        // v22.9: Also set Instagram-specific params so frontend has them
        redirectUrl.searchParams.set('connect_ig_username', instagramData.username || '');
        redirectUrl.searchParams.set('connect_ig_user_id', instagramData.igUserId);
        if (igAvatar) redirectUrl.searchParams.set('connect_ig_avatar', igAvatar);
        console.log('[Facebook OAuth] v22.9: ✅ Instagram connect:', instagramData.username);
        console.log('[Facebook OAuth] v22.9: Avatar sources → ig_profile_pic:', instagramData.profilePictureUrl ? 'YES' : 'NONE', '| fb_avatar:', facebookAvatar ? 'YES' : 'NONE', '| final_avatar:', igAvatar ? 'YES (' + igAvatar.substring(0, 60) + '...)' : 'NONE');
      } else if (isInstagramConnect && !instagramData.igUserId) {
        // User clicked "Connect Instagram" but no IG business account found
        // v22.9: Still pass Facebook avatar as fallback so the card shows SOME image
        redirectUrl.searchParams.set('connect_provider', 'instagram');
        redirectUrl.searchParams.set('connect_name', 'Instagram');
        // v22.9: Use Facebook avatar as fallback even when IG data is missing
        const fallbackAvatar = facebookAvatar || null;
        if (fallbackAvatar) redirectUrl.searchParams.set('connect_avatar', fallbackAvatar);
        redirectUrl.searchParams.set('connect_error', 'No Instagram business account found. Please ensure your Facebook Page has a linked Instagram account.');
        console.log('[Facebook OAuth] v22.9: ⚠️ Instagram connect but no IG data found — using FB avatar fallback:', fallbackAvatar ? 'YES' : 'NONE');
      } else {
        // User clicked "Connect Facebook" → set connect_provider=facebook
        ...
        console.log('[Facebook OAuth] v22.6: ✅ Facebook connect:', facebookName);
      }
      ...
      response.cookies.delete('oauth_instagram');
      return response;
    }
