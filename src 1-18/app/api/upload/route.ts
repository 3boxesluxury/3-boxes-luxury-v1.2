...
/** Ensure the storage bucket exists and is public. Creates it if missing. */
async function ensureBucket() {
  const { data: buckets } = await supabase.storage.listBuckets()
  const exists = buckets?.find((b) => b.name === STORAGE_BUCKET)
  if (exists) {
    // Make sure it's public so getPublicUrl works
    if (!exists.public) {
      await supabase.storage.updateBucket(STORAGE_BUCKET, { public: true })
    }
    return
  }
  // Create bucket — public, 5 MB limit, images only
  const { error } = await supabase.storage.createBucket(STORAGE_BUCKET, {
    public: true,
    fileSizeLimit: 5242880,
    allowedMimeTypes: ['image/*'],
  })
  if (error) console.error('[upload] Bucket creation failed:', error.message)
}
