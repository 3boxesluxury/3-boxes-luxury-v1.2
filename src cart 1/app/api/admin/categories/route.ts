...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch top-level categories with real product counts and children
    ...
    const transformed = categories.map((cat) => {
      const childProductCount = (cat.children || []).reduce((sum: number, child: any) => sum + child._count.products, 0)
      return {
        id: cat.id,
        name: cat.name,
        slug: cat.slug,
    ...
    return NextResponse.json({ categories: transformed, source: 'database' })
  } catch (err) {
