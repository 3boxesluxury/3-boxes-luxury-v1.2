...
export function PartnersTab({ token, onMutate }: { token: string | null; onMutate: () => void }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['partners', page, search, typeFilter, statusFilter],
    queryFn: () => apiFetch(`/api/partners?page=${page}&limit=20${search ? `&search=${search}` : ''}${typeFilter ? `&type=${typeFilter}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}`, undefined, token),
    retry: false,
    onError: (error: any) => setQueryError(error?.message || 'Failed to load partners'),
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => apiFetch(`/api/partners/${id}`, { method: 'DELETE' }, token),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['partners'] }); onMutate() },
  })

  const statusMut = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => apiFetch(`/api/partners/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }, token),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['partners'] }); onMutate() },
  })

  const verifyMut = useMutation({
    mutationFn: ({ id, isVerified }: { id: string; isVerified: boolean }) => apiFetch(`/api/partners/${id}`, { method: 'PATCH', body: JSON.stringify({ isVerified }) }, token),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['partners'] }); onMutate() },
  })
  ...
  return (
    <div className="space-y-4">
      ...
      {queryError && !isLoading ? (
        <Card className={cardCls}><CardContent className="py-8 text-center text-amber-200/70"><p className="font-semibold">Failed to load partners</p><p className="mt-2 text-sm text-amber-200/50">{queryError}</p></CardContent></Card>
      ) : isLoading ? (
...
function PartnerForm({ initial, token, onClose, onSaved }: { initial: Partner | null; token: string | null; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (initial) { await apiFetch(`/api/partners/${initial.id}`, { method: 'PUT', body: JSON.stringify(body) }, token) }
      else { await apiFetch('/api/partners', { method: 'POST', body: JSON.stringify(body) }, token) }
      onSaved()
