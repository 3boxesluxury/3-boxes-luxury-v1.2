...
export function HeroSection() {
  const { setView, setCategory, toggleGiftBuilder, setPendingScrollId } = useStore();
      ...
      <div className="relative container mx-auto px-4 py-8 sm:py-10 lg:py-12">
        ...
        >
          ...
          >
            <Button
              onClick={() => {
                trackBug('CLICK', '[Hero Shop Now] button pressed — queuing scroll to #products');
                setCategory(null);
                setView('home');
                // ✅ FIX: previously looked for #product-grid-section which DOES NOT EXIST
                // in the DOM (only #products exists in product-grid.tsx). So the scroll
                // silently failed. Now we use the same pendingScrollId mechanism as the
                // header so the scroll is handled by scrollToSectionById in header.tsx,
                // which uses scrollIntoView() and respects the scroll-mt-36 CSS class.
