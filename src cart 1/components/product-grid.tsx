...
const CATEGORY_SLUG_KEYS: Record<string, string> = {
  ...
  'couplefriendly': 'header.coupleFriendly',
  'home': 'header.homeDecor',
  ...
  'menaccessories': 'header.accessories',
  'menshirts': 'header.shirts',
  'mentshirts': 'header.tshirtsPolos',
  'menfragrances': 'header.menFragrances',
  'menwatches': 'header.menWatches',
  'menleather': 'header.menLeather',
  'womenjewelry': 'header.womenJewelry',
  'womensarees': 'header.womenSarees',
  'womenfashion': 'header.womenFashion',
  'womenfragrances': 'header.womenFragrances',
  'womenaccessories': 'header.womenAccessories',
  'homecandles': 'header.candlesFragrances',
  'homeliving': 'header.living',
  'officecorporategifts': 'header.corporateGifts',
  'officedesk': 'header.deskAccessories',
  'officestationery': 'header.stationery',
  'watches': 'categories.watches',
  'jewelry': 'categories.jewelry',
  'leather': 'categories.leatherGoods',
  'fragrances': 'categories.fragrances',
  'fashion': 'categories.fashion',
  'sarees': 'categories.sarees',
  'kids': 'categories.toys',
};
...
const SORT_OPTIONS = [
  ...
  { value: 'price-asc', labelKey: 'products.priceLowHigh' },
  { value: 'price-desc', labelKey: 'products.priceHighLow' },
  { value: 'rating', labelKey: 'products.topRated' },
];

const SOURCE_OPTIONS = [
  { value: 'all', labelKey: 'products.allSources' },
  { value: 'own', labelKey: 'products.ourCollection' },
  { value: 'external', labelKey: 'products.externalPlatforms' },
];
...
export function ProductGrid() {
  ...
  // BUG-TRACK: confirm #products section is mounted in DOM
  useEffect(() => {
    const el = document.getElementById('products');
    trackBug('RENDER', `ProductGrid mounted — #products element present: ${!!el}`);
    if (el) {
      trackBug('RENDER', '#products element bounding rect', el.getBoundingClientRect().toJSON());
    }
    return () => {
      trackBug('RENDER', 'ProductGrid unmounting — #products element will be removed');
    };
  }, []);

  // BUG-TRACK: log when filters change
  useEffect(() => {
    trackBug('STATE', `ProductGrid filter changed — category="${selectedCategory}" search="${searchQuery}" sort="${sort}"`);
  }, [selectedCategory, searchQuery, sort, sourceFilter, platformFilter, occasionFilter, recipientFilter, relationshipFilter, priceRangeFilter]);
  ...
  return (
    <section id="products" className="scroll-mt-36 py-8">
      <div className="mb-6 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h2 className="text-2xl font-bold text-amber-100">
              {searchQuery ? t('products.resultsFor', { query: searchQuery }) : selectedCategory ? t(CATEGORY_SLUG_KEYS[selectedCategory.replace(/-/g, '').toLowerCase()] || 'products.allProducts') : t('products.allProducts')}
            </h2>
            <p className="mt-1 text-sm text-amber-200/50">
              {t('products.productsFound', { count: data?.total ?? 0 })}
            </p>
          </div>
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-2 rounded-lg border border-amber-900/20 bg-stone-900/50 p-3">
            <Select value={sourceFilter} onValueChange={setSourceFilter}>
              <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.source')} /></SelectTrigger>
              <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
                {SOURCE_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{t(opt.labelKey)}</SelectItem>)}
              </SelectContent>
            </Select>
            {availablePlatforms.length > 0 && (
              <Select value={platformFilter} onValueChange={setPlatformFilter}>
                <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.platform')} /></SelectTrigger>
                <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
                  <SelectItem value="all">{t('products.all')} {t('products.platform')}</SelectItem>
                  {availablePlatforms.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
                </SelectContent>
              </Select>
            )}
            <Select value={occasionFilter} onValueChange={setOccasionFilter}>
              <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.occasion')} /></SelectTrigger>
              <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
                <SelectItem value="all">{t('products.allOccasions')}</SelectItem>
                {OCCASION_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={recipientFilter} onValueChange={setRecipientFilter}>
              <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.recipient')} /></SelectTrigger>
              <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
                <SelectItem value="all">{t('products.allRecipients')}</SelectItem>
                {RECIPIENT_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={relationshipFilter} onValueChange={setRelationshipFilter}>
              <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.relationship')} /></SelectTrigger>
              <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
                <SelectItem value="all">{t('products.anyRelationship')}</SelectItem>
                {RELATIONSHIP_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={priceRangeFilter} onValueChange={setPriceRangeFilter}>
              <SelectTrigger className="w-[140px] border-amber-900/40 bg-stone-900/50 text-amber-200/70"><SelectValue placeholder={t('products.price')} /></SelectTrigger>
              <SelectContent className="border-amber-900/40 bg-stone-950 text-amber-100">
            ...
            {hasActiveFilters && (
              <Button variant="ghost" size="sm" onClick={clearFilters} className="text-amber-300/70 hover:bg-amber-900/20 hover:text-amber-200">
                <X className="mr-1 h-4 w-4" /> {t('common.clear')}
              </Button>
            )}
          </div>
        )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-2 text-sm text-amber-200/40">{t('products.tryAdjusting')}</p>
