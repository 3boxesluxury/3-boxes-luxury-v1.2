...
interface AppState {
  ...
  // ✅ Global pending-scroll target ID. Set by any component (hero, footer,
  // header nav tabs) to queue a scroll that will be executed by the header's
  // scrollToSectionById effect once the target element is in the DOM.
...
export const useStore = create<AppState>()(
  ...
  addItem: (item) =>
    set((state) => {
      // Prevent guest cart — user must be authenticated
      if (!state.authUser) return state
      const qty = item.quantity || 1 // Use provided quantity, default to 1
      ...
      return { cartItems: [...state.cartItems, { ...itemWithoutQuantity, quantity: qty }] }
    }),
  ...
  clearAuth: () => {
    ...
    // Clear cart on logout to prevent guest cart merging with authenticated cart
    set({ authUser: null, authToken: null, authView: null, authTwoFAStep: false, authPendingUserId: null, authTwoFAMethod: null, authPendingEmail: null, cartItems: [] })
  },
  ...
  // setPendingScrollId is declared above with the initial state
  setAppTheme: (theme) => {
    try {
