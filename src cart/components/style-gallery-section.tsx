...
const STATUS_BADGE_CONFIG: Record<string, { color: string; icon: any }> = {
  pending: { color: 'bg-amber-600/20 text-amber-300 border-amber-600/30', icon: Clock },
  approved: { color: 'bg-emerald-600/20 text-emerald-300 border-emerald-600/30', icon: CheckCircle },
  rejected: { color: 'bg-red-600/20 text-red-300 border-red-600/30', icon: XCircle },
};
    ...
    async function fetchGallery() {
      try {
        ...
        if (res.ok) {
          ...
          // API returns { images: [...] } — normalize to our GalleryItem shape
          const rawImages = Array.isArray(data?.images) ? data.images : [];
          const mapped: GalleryItem[] = rawImages.map((img: any) => ({
            id: img.id || '',
            productId: img.productId || '',
            productName: img.product?.name || t('aiGallery.luxuryProduct'),
            productImage: img.product?.images
              ? (Array.isArray(img.product.images) ? img.product.images[0] : (() => { try { const p = JSON.parse(img.product.images); return Array.isArray(p) ? p[0] : img.product.images; } catch { return img.product.images; } })())
              : undefined,
            userName: img.userName || 'Anonymous',
            aiGeneratedImage: img.aiGeneratedImage || '',
            categorySlug: img.product?.slug || undefined,
            likes: img.likes || 0,
            status: img.isApproved ? 'approved' : img.isActive ? 'pending' : 'rejected',
            createdAt: img.createdAt || new Date().toISOString(),
          }));
          setItems(mapped);
        }
      } catch {
        // ignore — gracefully show empty state
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-amber-700/30 bg-amber-900/20 px-4 py-1.5 mb-4">
            ...
            <span className="text-xs font-semibold text-amber-300 uppercase tracking-wider">{t('styleGallery.title')}</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-bold text-amber-100 mb-3">
            {t('styleGallery.heading')}
          </h2>
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-amber-200">{t('styleGallery.mySubmissions')}</span>
                {mySubmissions.filter(s => s.status === 'pending').length > 0 && (
                  <Badge className="bg-amber-600/20 text-amber-300 border-amber-600/30 text-[10px] px-1.5 py-0.5">
                    {mySubmissions.filter(s => s.status === 'pending').length} {t('styleGallery.pending')}
                  </Badge>
                )}
              </div>
            ...
            {showMySubmissions && (
              <div className="mt-2 space-y-2 rounded-xl border border-amber-900/20 bg-stone-900/40 p-3">
                {mySubmissions.map(sub => {
                  const statusCfg = STATUS_BADGE_CONFIG[sub.status] || STATUS_BADGE_CONFIG.pending;
                  const StatusIcon = statusCfg.icon;
                  const statusLabel = sub.status === 'pending' ? t('styleGallery.underAdminApproval') : sub.status === 'approved' ? t('styleGallery.approved') : t('styleGallery.rejected');
                  return (
        ...
        {/* Gallery Grid — single column on mobile (Issue 5) */}
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-md sm:max-w-none mx-auto sm:mx-0">
          ...
          >
            ...
            <p className="text-base font-medium text-amber-200/50 mb-1">{t('styleGallery.emptyTitle')}</p>
            <p className="text-sm text-amber-200/30 max-w-md text-center mb-6">
              {t('styleGallery.emptyDesc')}
            </p>
            <div className="flex items-center gap-2 text-xs text-amber-200/25">
              <Shield className="h-3.5 w-3.5" />
              <span>{t('styleGallery.reviewNote')}</span>
            </div>
          </motion.div>
        )}
        ...
        >
          ...
          <Button
            onClick={() => {
              ...
              // Set flag to prevent page.tsx from scrolling to top
              (window as any).__pendingScrollToSection = true;

              setView('home');

              // Use retry loop to wait for the products section to render
              let attempts = 0;
              const tryScroll = () => {
                attempts++;
