...
export type View = 'home' | 'product' | 'cart' | 'checkout' | 'orders' | 'order-confirmation' | 'user-dashboard' | 'admin-dashboard' | 'agent-dashboard' | 'team-dashboard' | 'corporate-dashboard' | 'wiki' | 'downloads' | 'security-policy' | 'social-style' | '3box-curate' | 'family-shopping' | 'wishlist'
...
// WishlistItem - stored locally in the Zustand store (no auth required).
// Mirrors the key fields of CartItem so a wishlist item can be moved to cart easily.
export interface WishlistItem {
  ...
  rating?: number
  stock?: number
  addedAt: number  // Unix timestamp - for sorting wishlist by most-recently-added
}
...
function loadWishlistFromStorage(): WishlistItem[] {
  ...
  try {
    const stored = localStorage.getItem('3boxes_wishlist')
    if (stored) {
      const parsed = JSON.parse(stored)
...
export const useStore = create<AppState>((set, get) => ({
  ...
  addToWishlist: (item) =>
    set((state) => {
      // Avoid duplicates - if already in wishlist, do nothing
      if (state.wishlistItems.some((w) => w.productId === item.productId)) {
        return state
      }
      const newWishlist = [...state.wishlistItems, { ...item, addedAt: Date.now() }]
      try {
        localStorage.setItem('3boxes_wishlist', JSON.stringify(newWishlist))
      } catch {}
      return { wishlistItems: newWishlist }
    }),
  removeFromWishlist: (productId) =>
    set((state) => {
      const newWishlist = state.wishlistItems.filter((w) => w.productId !== productId)
      try {
        localStorage.setItem('3boxes_wishlist', JSON.stringify(newWishlist))
      } catch {}
      return { wishlistItems: newWishlist }
  ...
  clearWishlist: () => {
    try { localStorage.removeItem('3boxes_wishlist') } catch {}
    set({ wishlistItems: [] })
  },
  isInWishlist: (productId) => get().wishlistItems.some((w) => w.productId === productId),
