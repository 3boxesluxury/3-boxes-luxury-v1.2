...
export function StyleGalleryTab() {
  ...
  const fetchAllItems = useCallback(async () => {
    ...
    try {
      ...
      // Single request for all admin gallery items reduces load and latency.
      const res = await fetch('/api/style-gallery?limit=100', { headers })
      if (!res.ok) throw new Error('Failed to load gallery items')

      const data = await res.json()
      const allItems: GalleryImage[] = data.images || []

      setPendingItems(allItems.filter(item => item.isActive && !item.isApproved))
      setApprovedItems(allItems.filter(item => item.isActive && item.isApproved))
      setRejectedItems(allItems.filter(item => !item.isActive))
    } catch {
