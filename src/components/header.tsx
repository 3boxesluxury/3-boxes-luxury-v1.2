...
export function Header() {
  const { searchQuery, setSearch, setView, view, cartItems, wishlistItems, setCategory, selectedCategory, authUser, setAuthView, clearAuth, toggleGiftBuilder, appTheme, setAppTheme, scrollToProducts, locale, setLocale, currency, setCurrency } = useStore();
          ...
          <div className="flex flex-col">
            ...
            <div className="border-b border-amber-900/20 py-2">
              ...
              {CATEGORY_NAV.filter((cat) =>
                ['couple', 'men', 'women', 'kids', 'home', 'office', 'new-arrivals', 'family-packs'].includes(cat.slug)
              ).map((cat) => {
                const Icon = cat.icon;
            ...
            <div className="border-b border-amber-900/20 py-2">
              ...
              >
                ...
                <span className="flex-1">Gift Builder</span>
          ...
          <nav className="hidden md:flex items-center justify-between w-full" aria-label="Category navigation">
            {CATEGORY_NAV.map((cat) => {
              ...
              const handleNavClick = () => {
                if (cat.scrollToId) {
                  // Navigate to home and queue the scroll. The actual scroll is
                  // performed by the useEffect above once the home view has
                  // rendered, so the target section is guaranteed to exist.
                  setView('home');
                  setCategory(null);
                  setPendingScrollId(cat.scrollToId);
                } else if (cat.viewId) {
