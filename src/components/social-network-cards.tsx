      ...
      {SOCIAL_NETWORKS.map((networkConfig) => {
        ...
        return (
          <motion.div key={networkConfig.network} variants={itemVariants} className="h-full">
            <div
              className={`flex h-full flex-col items-center rounded-xl border p-3 text-center transition-all duration-200 sm:p-4 ${
                isConnected
                  ? `${networkConfig.borderColor} bg-stone-900/80 hover:border-amber-700/40`
                  : 'border-amber-900/30 bg-stone-900/80 hover:border-amber-900/50 hover:bg-stone-900'
              }`}
            >
              {/* Brand icon in a color-coded circle */}
              <div
              ...
              <p className="mt-0.5 w-full truncate text-xs text-amber-200/50">
              ...
              <div className="mt-2 flex items-center gap-1.5">
                ...
                <span
                  className={`text-[10px] font-medium uppercase tracking-wide ${
                    isConnected ? 'text-emerald-400' : 'text-stone-400'
                  }`}
                >
              ...
              <div className="mt-3 w-full">
                {isConnecting ? (
                  <Button
                    disabled
                    size="sm"
                    className="w-full gap-1 bg-stone-800 text-xs text-amber-200/60"
                  >
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="w-full gap-1 border-amber-900/40 text-xs text-amber-200/70 hover:border-red-900/40 hover:bg-red-900/20 hover:text-red-300"
                  >
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className={`w-full gap-1 border text-xs ${networkConfig.color} ${networkConfig.bgColor} ${networkConfig.borderColor} hover:opacity-80`}
                  >
