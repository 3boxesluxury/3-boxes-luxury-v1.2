...
interface SocialConnection {
  ...
  profile?: {
    ...
    avatar?: string
    email?: string
    gender?: string  // v16: gender from OAuth provider
  }
  likes?: { name: string; category: string }[]
  // v17: Facebook extended data for AI analysis
  fbData?: {
    gender?: string | null
    ...
    likes?: { name: string; category: string }[]
    likesCount?: number
    fashionLikes?: { name: string; category: string }[]
  }
}
...
function loadPersistedConnections(): Record<string, { name: string; email?: string; avatar?: string; gender?: string }> {
...
function persistConnections(data: Record<string, { name: string; email?: string; avatar?: string; gender?: string }>) {
  if (typeof window === 'undefined') return
  try {
...
export function SocialStyleIntegration() {
  ...
  const updateConnection = useCallback(
    (network: SocialNetwork, updates: Partial<SocialConnection>) => {
      setConnections((prev) => {
        ...
        const persisted: Record<string, { name: string; email?: string; avatar?: string; gender?: string }> = {}
        for (const c of next) {
          if (c.connected && c.profile) {
            persisted[c.network] = { name: c.profile.name, email: c.profile.email, avatar: c.profile.avatar, gender: c.profile.gender }
          }
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      // v17: Read Facebook extended data (photos, likes, birthday, age, fashionLikes)
      const urlConnectFbData = urlParams.get('connect_fb_data')
      let parsedFbData: SocialConnection['fbData'] = undefined
      if (urlConnectFbData) {
        try {
          parsedFbData = JSON.parse(decodeURIComponent(urlConnectFbData))
          console.log('[Social Connect] v17: Parsed Facebook extended data from URL — photos:', parsedFbData?.photosCount, 'likes:', parsedFbData?.likesCount, 'ageGroup:', parsedFbData?.ageGroup)
        } catch (e) {
          console.warn('[Social Connect] v17: Failed to parse connect_fb_data from URL:', e)
        }
      }
      ...
      if (urlConnectProvider && urlConnectName && authUser) {
        ...
        if (network === 'google' || network === 'facebook' || network === 'linkedin') {
          updateConnection(network, {
            ...
            profile: {
              ...
              // v15: Store gender from OAuth provider
              gender: urlConnectGender || undefined,
            },
            // v17: Store Facebook extended data for AI analysis
            ...(parsedFbData && network === 'facebook' ? { fbData: parsedFbData } : {}),
          })
        }
        ...
        // v17: Clean up Facebook extended data
        sessionStorage.removeItem('3boxes_oauth_fb_data')
      ...
      // v17: Read Facebook extended data from sessionStorage
      let oauthFbData: SocialConnection['fbData'] = undefined
      try {
        const rawFbData = sessionStorage.getItem('3boxes_oauth_fb_data')
        if (rawFbData) {
          oauthFbData = JSON.parse(rawFbData)
          console.log('[Social Connect] v17: Found Facebook extended data in sessionStorage — photos:', oauthFbData?.photosCount, 'likes:', oauthFbData?.likesCount, 'ageGroup:', oauthFbData?.ageGroup)
        }
      } catch (e) {
        console.warn('[Social Connect] v17: Failed to parse fb_data from sessionStorage:', e)
      }
      ...
      if (oauthProvider && authUser) {
        ...
        if (network === 'google' || network === 'facebook' || network === 'linkedin') {
          updateConnection(network, {
            ...
            // v17: Store Facebook extended data for AI analysis
            ...(oauthFbData && network === 'facebook' ? { fbData: oauthFbData } : {}),
          })
        }
        ...
        sessionStorage.removeItem('3boxes_oauth_gender')
        // v17: Clean up Facebook extended data from sessionStorage
        sessionStorage.removeItem('3boxes_oauth_fb_data')
        sessionStorage.removeItem('3boxes_linkedin_connect')
        sessionStorage.removeItem('3boxes_linkedin_connect_time')
        sessionStorage.removeItem('3boxes_google_connect')
  ...
  const handleAnalyzeStyle = useCallback(async () => {
    ...
    // Include Facebook profile data + v17 extended data (photos, likes, birthday, age, fashionLikes, gender)
    ...
    if (fbConnection?.profile) {
      ...
      // v17: Use fbData from connection if available (stored by OAuth callback), fallback to cookie/sessionStorage
      const fbExt = fbConnection.fbData
      socialPayload.facebookData = {
        profile: fbConnection.profile,
        // v17: Pass ALL likes from Facebook API (not just cookie-based ones)
        likes: fbExt?.likes || fbRich?.likes || fbConnection.likes || [],
        // v17: Pass age data (birthday, ageRange, ageGroup)
        ...(fbExt?.birthday && { birthday: fbExt.birthday }),
        ...(fbExt?.ageRange && { ageRange: fbExt.ageRange }),
        ...(fbExt?.ageGroup && { ageGroup: fbExt.ageGroup }),
        // v17: Pass photo URLs for VLM fashion analysis
        ...(fbExt?.photos && fbExt.photos.length > 0 && { photos: fbExt.photos }),
        // v17: Pass fashion-specific likes for brand matching
        ...(fbExt?.fashionLikes && fbExt.fashionLikes.length > 0 && { fashionLikes: fbExt.fashionLikes }),
        ...(fbRich?.inferredSignals && { inferredSignals: fbRich.inferredSignals }),
        // v15/v17: Pass gender from Facebook OAuth
        ...(fbExt?.gender && { gender: fbExt.gender }),
        ...(!fbExt?.gender && fbConnection.profile.gender && { gender: fbConnection.profile.gender }),
      }
    }
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <CardDescription className="text-amber-200/50 line-clamp-2 min-h-[2.5rem]">
                              {isConnected
