...
// v16: Name-based gender inference for LinkedIn (API doesn't provide gender)
// LinkedIn's OpenID Connect endpoint removed gender in 2023, so we infer from first name.
const MALE_NAMES = new Set([
  ...
  'aaron','jose','nathan','henry','peter','adam','zachary','walter','kyle','harold',
  'carl','arthur','gerald','roger','keith','jeremy','terry','lawrence','sean','christian',
  'albert','joe','ethan','austin','jesse','willie','billy','bryan','bruce','jordan',
  'ralph','roy','noah','eugene','carlos','russell','bob','victor','martin','ernest',
  'phillip','todd','jared','cameron','chad','alan','philip','johnny','owen','lukas',
  'aditya','arjun','rahul','vikram','amit','suresh','rajesh','anil','sanjay','manoj',
  'prakash','deepak','ravi','mohan','sunil','ashok','kiran','naresh','dev','raj',
  'mohammed','ali','omar','ahmed','hassan','ibrahim','yusuf','khalid','faisal','hamza',
  'li','wei','zhang','ming','jun','hao','yong','lei','peng','tao',
  'takeshi','kenji','hiroshi','yuki','akira','shin','taro','ryu','kazuki','daichi',
]);
...
const FEMALE_NAMES = new Set([
  ...
  'teresa','ann','sara','madison','frances','kathryn','janice','jean','abby','jessie',
  'priya','anita','meena','neha','pooja','sunita','rekha','divya','anju','manisha',
  'swati','nisha','kavita','sangeeta','rupa','aarti','shweta','prachi','gayatri','usha',
  'fatima','aisha','maryam','noor','zainab','sana','hira','amna','sumbal','iqra',
  'ling','mei','xiao','yan','hua','jing','fang','hong','li','na',
  'yuki','sakura','miyuki','haruka','naomi','ayumi','mai','rika','akiko','emi',
]);

function inferGenderFromName(firstName: string): 'male' | 'female' | null {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // v16: LinkedIn API doesn't provide gender — infer from first name
    // This covers common Western, Indian, Arabic, and East Asian names.
    // If name inference fails, the social-style route will fall back to VLM detection.
    const linkedinGender = inferGenderFromName(linkedinFirstName || linkedinName?.split(' ')[0] || '');
    console.log('[LinkedIn OAuth] v16: Gender inferred from name:', linkedinGender || 'could not determine');
    ...
    if (isConnectAction) {
      ...
      // v16: Pass inferred gender so social-style route can use it
      if (linkedinGender) redirectUrl.searchParams.set('connect_gender', linkedinGender);
      ...
      if (user) {
        ...
        user = await db.user.update({
          ...
          data: {
            ...
            // v16: Persist gender from LinkedIn name inference
            ...(linkedinGender && !user.gender ? { gender: linkedinGender } : {}),
          },
        });
      }
    }
      ...
      if (!linkedinEmail) {
        ...
        user = await db.user.create({
          data: {
            ...
            // v16: Persist gender from LinkedIn name inference
            ...(linkedinGender ? { gender: linkedinGender } : {}),
          },
        });
      } else {
        user = await db.user.create({
    ...
    const jwtToken = jwt.sign(
      {
        ...
        name: user.name,
        role: user.role,
        gender: user.gender,  // v16: persisted gender from OAuth
      },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
    ...
    // v16: Pass inferred gender from LinkedIn in login redirect
    if (linkedinGender) redirectUrl.searchParams.set('auth_gender', linkedinGender);
