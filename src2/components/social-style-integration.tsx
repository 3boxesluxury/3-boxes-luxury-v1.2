...
export function SocialStyleIntegration() {
  ...
  const handleFacebookConnect = useCallback(() => {
    ...
    window.location.href = `/api/auth/facebook?returnTo=${encodeURIComponent(returnUrl)}`
  }, [updateConnection])
  ...
  const handleLinkedInConnect = useCallback(() => {
    ...
    const returnUrl = '/?view=social-style'
    window.location.href = `/api/auth/linkedin?returnTo=${encodeURIComponent(returnUrl)}`
  }, [updateConnection])
  ...
  // When OAuth redirects back to this page (via ?view=social-style), the
  // OAuthCallbackHandler sets the auth state AND stores provider info in sessionStorage.
  // We read from sessionStorage + authUser to reliably mark the social network as "connected".
  //
  // IMPORTANT: We do NOT clear the sessionStorage markers until the connection
  // is established. This handles the race condition where OAuthCallbackHandler
  // cleans the URL before this component's useEffect can read the auth params.

  useEffect(() => {
    if (typeof window === 'undefined') return

    try {
      // Read OAuth provider info stored by OAuthCallbackHandler
      ...
      // Also check the old per-provider sessionStorage markers (set by handle*Connect)
      ...
      // If OAuthCallbackHandler stored provider info AND we have an authenticated user,
      // mark the corresponding social network as connected
      if (oauthProvider && authUser) {
        const profileName = oauthName || authUser.name
        const profileEmail = oauthEmail || authUser.email
      ...
      // Fallback: If no oauthProvider in sessionStorage but we have per-provider markers
      // and the user is authenticated, mark as connected using authUser info
      ...
      // (authUser will be updated after OAuthCallbackHandler runs)
